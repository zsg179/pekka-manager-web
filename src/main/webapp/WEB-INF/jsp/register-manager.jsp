...
<div class="w" id="regist">
    <div class="mt">
        <ul class="tab">
            <li class="curr">管理员</li>
        </ul>
        <div class="extra">
        <span>我已经注册，现在就&nbsp;
        	<a href="http://localhost:8081/page/login" class="flk13">登录</a>
        </span>
        </div>
    </div>
    <div class="mc">
                ...
                <div class="item" id="select-regName">
                    ...
                    <div class="fl item-ifo">
                        <div class="o-intelligent-regName">
                            <input type="text" id="regName" name="managerName" class="text" tabindex="1" autoComplete="off"
...
<script type="text/javascript">
	var REGISTER={
		...
		beforeSubmit:function() {
				//检查用户是否已经被占用
				$.ajax({
	            	url : REGISTER.param.surl + "/manager/check/"+escape($("#regName").val())+"/1?r=" + Math.random(),
	            	success : function(data) {
	            		if (data.data) {
	            			//检查手机号是否存在
	            			$.ajax({
	            				url : REGISTER.param.surl + "/manager/check/"+$("#phone").val()+"/2?r=" + Math.random(),
				            	success : function(data) {//手机号没有被注册，现在可以用
				            		if (data.data) {
				            			//检查邮箱是否存在
				            			$.ajax({
				            				url : REGISTER.param.surl + "/manager/check/"+$("#email").val()+"/3?r=" + Math.random(),
		...
		doSubmit:function() {
			...
			$.post("/manager/register",$("#personRegForm").serialize(), function(data){
				if(data.status == 200){
					alert('管理员注册成功，请登录！');
					REGISTER.login();
				} else {
